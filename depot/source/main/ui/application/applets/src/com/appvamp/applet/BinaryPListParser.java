 ...
 * @version $Id: BinaryPListParser.java 225 2010-06-18 14:05:16Z wrandelshofer $
 */
public class BinaryPListParser {
	...
	public XMLElement parse(byte[] raw_bytes) throws IOException {
	    int size = raw_bytes.length;
    	DataInputStream raf = new DataInputStream(new ByteArrayInputStream(raw_bytes));
        ...
        try {
           // raf = new RandomAccessFile(file, "r");
			...
			DataInputStream raf2 = new DataInputStream(new ByteArrayInputStream(raw_bytes, size - 32, 32));
...
//            raf.seek(raf.length() - 32);
            //	count of offset ints in offset table
            offsetCount = (int)raf2.readLong();
            //  count of object refs in arrays and dicts
            refCount = (int) raf2.readLong();
            //  count of offsets in offset table (also is number of objects)
            objectCount = (int) raf2.readLong();
            //  element # in offset table which is top level object
